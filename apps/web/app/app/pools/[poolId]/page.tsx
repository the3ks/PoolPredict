...
import { Market, PoolSummary, Prediction, TournamentEvent } from "../../../lib/types";
...
export default function PoolOverviewPage() {
  ...
  async function submitPrediction(event: FormEvent<HTMLFormElement>) {
    ...
    setSelectedOption("");
    await Promise.all([loadPredictions(pool.id), loadBalance(pool.id)]);
  }
  ...
  return (
    <section className="pageStack">
      <PageHeader
        ...
        icon={Waves}
        actions={pool ? <Link className="button buttonSecondary" href={`/app/pools/${pool.id}/invites`}><IconLabel icon={KeyRound}>Invites</IconLabel></Link> : null}
      />
