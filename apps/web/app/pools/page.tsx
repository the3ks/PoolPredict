...
import { BadgeDollarSign, CircleDotDashed, Hash, Plus, Send, ShieldCheck, Trophy, UserPlus, Users } from "lucide-react";
...
export default function PoolsPage() {
  ...
  async function loadPools() {
    ...
    try {
      ...
      if (!myPoolsResponse.ok) {
        setStatus(await readApiError(myPoolsResponse, "Could not load your pools."));
        return;
      }

      if (!otherPoolsResponse.ok) {
        setStatus(await readApiError(otherPoolsResponse, "Could not load other pools."));
        return;
      }
      ...
      setStatus(`${myPools.length} joined or owned. ${discoverPools.length} available to request.`);
    } catch (error) {
  ...
  async function requestJoin(pool: DiscoverPool) {
    ...
    try {
      const response = await fetch(apiUrl(`/api/pools/${pool.id}/join-requests`), {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });

      if (!response.ok) {
        setRequestStatus(await readApiError(response, "Could not request to join."));
        return;
      }
      ...
      setRequestStatus(error instanceof Error ? error.message : "Could not request to join.");
    }
  }
  ...
  return (
    ...
    <section className="pageStack">
      ...
      {requestStatus ? <StatusPill icon={Send}>{requestStatus}</StatusPill> : null}
      <Panel title="Your Pools">
        <div className="poolList">
          {pools.length === 0 ? <p className="mutedText">You have not joined or created any pools yet.</p> : null}
          {pools.map((pool) => (
      ...
      <Panel title="Other Pools">
        <div className="poolList">
          {otherPools.length === 0 ? <p className="mutedText">No other pools are available right now.</p> : null}
          {otherPools.map((pool) => (
            <article className="poolCard" key={pool.id}>
              <div className="poolCardLink discoverPoolCardLink">
                ...
                <span>
                  ...
                  <small><IconLabel icon={Trophy}>{pool.provider}{pool.isTestData ? " test data" : ""}</IconLabel></small>
                </span>
                <span>
                  <strong>{pool.memberCount}</strong>
                  <small><IconLabel icon={Users}>members</IconLabel></small>
                ...
                <span>
                  ...
                  <small><IconLabel icon={BadgeDollarSign}>starting balance</IconLabel></small>
                </span>
                <button
                ...
                >
                  <IconLabel icon={Send}>{pool.hasPendingJoinRequest ? "Requested" : "Request to join"}</IconLabel>
                </button>
              </div>
            </article>
          ))}
